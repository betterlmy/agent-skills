...
Loaded on demand through `style-presets.md` when the user asks to learn a style ("learn my style from `<path>` as `<name>`") or when the agent needs to render a sample after extraction.
...
2. Export a clean preview without embedded XML: `draw.io -x -f png -s 2 -o <preset-name>-sample.png <tmp>.drawio`.
...
**Prerequisite:** the agent's vision capability must be available, as described in `workflow.md`. If vision is not available, stop and tell the user:
...
1. **Read the image.** Use the agent's vision input — the same mechanism used for preview self-check in `workflow.md`.
