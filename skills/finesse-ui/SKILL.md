...
本 Skill 的 `audit` 命令自动化路径依赖 Node.js 运行时（`scripts/detect.mjs`，无第三方依赖）。首次运行前先读 [CLI 兼容性契约](references/cli-compatibility.md)，完成其中的版本与能力探测；Node 缺失或契约未通过时按 `references/audit.md` 的人工清单降级。
