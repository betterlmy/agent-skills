...
- 只有仓库明确选择文件化本地配置时才增加 `.env` 加载；先检查现有入口、启动脚本和部署注入方式，不把 dotenv 当成所有 Go 项目的默认依赖。
- 需要引入库时优先使用体积小且职责单一的方案，例如显式调用 `github.com/joho/godotenv.Load`；固定版本并补启动边界测试，不使用仅靠空白导入触发的 `godotenv/autoload`。
- 默认把 `.env` 视为可选的本地开发输入：文件不存在时继续使用进程环境，文件存在但无效或不可读时尽早失败；错误不得回显可能含凭据的文件内容。
- 进程、容器、CI 和部署系统已经注入的环境变量必须优先，禁止用 `Overload` 静默覆盖。生产运行不应依赖当前工作目录恰好存在 `.env`。
- 将 `.env` 和本地变体加入忽略规则，只提交无真实凭据的 `.env.example`；示例覆盖必填项、标明必须替换的值，并说明复制、加载、优先级和生产边界。
- 测试至少覆盖文件缺失、成功加载、外部环境优先和非法文件安全失败；涉及进程全局环境时避免并行测试并在结束后恢复状态。
