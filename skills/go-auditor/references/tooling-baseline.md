...
- 脚本需要 Bash、`find`、`sed`、`wc` 和 `mktemp`。
...
- 使用 `-mod=readonly`；存在根级 `vendor/modules.txt` 且不是 workspace 时使用 `-mod=vendor`。根目录存在 `go.work` 时固定使用该文件，否则设置 `GOWORK=off`，避免继承仓库外工作区。
...
| 执行失败 | 工具退出非零，可能是代码问题、依赖缺失或环境错误；需结合输出分类 |
...
脚本最多展示每项检查前 400 行输出。报告必须回到完整的 CI 日志或按需复跑确认，不能将截断输出视为完整命中数。
...
这些命令可能耗时、访问外部服务或依赖仓库专用环境，不能由基线脚本自动执行。只有目标仓库授权后，才按其 Makefile 或 CI 约定运行。例如：
...
- 每个工具的状态、实际命令、关键输出与失败分类。
- 格式或 linter 命中数；输出截断时明确标注。
- 未安装、未授权或因离线依赖无法执行的项目。
- 用户范围、当前 Go 文件数、工具实际扩展的 package 范围，以及无法扫描的删除路径。
