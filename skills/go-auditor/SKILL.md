...
读 `references/tooling-baseline.md`，根据 `references/audit-scope.md` 生成 `scripts/scan.sh` 参数。脚本默认离线、模块只读、不执行测试。`quick` 完成本步和范围摘要后停止。
...
读 `references/report-format.md`，按风险、影响面和修复成本排序。报告必须声明用户范围、为理解行为读取的上下文范围、未检查区域和不可外推的结论。
...
3. 工具缺失、命令失败或未获授权时标注“未运行”或“执行失败”，不得写成通过。
