...
把"复刻一个网站"做成可重复的流程。默认在当前工作目录（即当前项目目录）工作：`NOTES.md`、`RECON/`、`CLONE_REPORT.md`、`CLONE_AUDIT.md` 和最终可预览的 `index.html` 都应写在当前项目内，除非用户明确指定外部工作目录。

**运行环境准备（跑任何 `scripts/` 前先看）**：
- 文中命令写的 `node scripts/xxx.mjs` 都相对**本 `SKILL.md` 所在目录**解析（即 `<skill 目录>/scripts/`），不是相对当前工作目录。先取本 Skill 目录的绝对路径再调用，例如 `node "<skill 目录>/scripts/recon-site.mjs" ...`；`RECON/`、`assets/` 等产物仍写到当前项目目录。
- 脚本内置基于 Chrome DevTools Protocol 的零依赖控制层，直接复用系统已有的 Chrome、Edge 或 Chromium，**不需要 Playwright，也不下载浏览器**。**禁止在用户项目里执行 `npm install playwright` 或下载 Chromium。** 若预检报告没有兼容浏览器，直接说明缺少系统浏览器，或通过环境变量 `WEB_CLONE_BROWSER_PATH` 指定可执行文件路径；不要改为目测，也不要反复安装依赖。
- 本 skill 强依赖外部运行时与 CLI：`node`（运行 `scripts/*.mjs`）、系统 Chrome/Edge/Chromium（CDP）以及 `gh` CLI（Step 1 的 GitHub 源码检索）。跑任何 `scripts/` 前先阅读 [CLI 兼容性契约](references/cli-compatibility.md)，探测本机版本与关键能力；`gh` 缺失时按契约降级为网页版 GitHub 搜索或 `curl https://api.github.com`。
...
```bash
# 按站名/产品名搜（用 gh 自带的 --jq，不依赖外部 jq；gh 缺失时见 references/cli-compatibility.md 的降级路径）
gh api "search/repositories?q=<关键词>" \
  --jq '.items[] | "\(.full_name) ⭐\(.stargazers_count) \(.description)"' | head -10
...
| 侦察结果 | 走哪条路 |
|---|---|
| 静态 HTML/CSS，无框架 | `wget --mirror` 抓镜像（未装 wget 时改用 `scripts/mirror-site.mjs` 或 `curl` 逐页抓）→ 删追踪脚本 → 改文案 |
...
| **WebGL / Canvas / Three.js 重前端** | **深度逆向真源码（见下）→ 忠实复刻 或 找同类开源 3D 模板换内容**。单文件原生站常常逐字节保留=最忠实复刻。**找不到真源码时走运行时帧捕获 + baseline 闸门**，纪律见 `references/effect-extraction.md` |
...
- **预览/子路径适配**：交付前把项目根资源引用改成相对路径，避免 `/assets/...` 这类根绝对路径在嵌套入口文件、文件预览或子路径部署下直接 404：

```bash
node scripts/preview-rewrite.mjs --project .
```
...
详见 `references/effect-extraction.md`（含运行时捕获兜底 + 何时走帧捕获兜底路线）。
...
```bash
gh api repos/<u>/<r> --jq '.license'  # + 找 LICENSE 文件 + 看 README
```
...
- 如用户有外部复刻索引，再按需更新该索引；本 skill 不强制维护全局中枢 README。
...
- `scripts/preview-rewrite.mjs`：把 HTML/CSS/SVG 里的项目根资源引用（如 `/assets/main.css`）改成相对路径，保证克隆站在嵌套入口文件、文件预览或子路径部署下仍能加载资源。
