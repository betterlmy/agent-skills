...
遇到下面情况，按第四节的工具方向自己做运行时帧捕获，先产出"最小可复现 baseline + 证据包"，再进入工程化：

- 站是 WebGL/WebGPU/重 Canvas 特效，且 GitHub + source-map 都找不到真源码；
- 需要运行时帧捕获、逐帧比对、shader/uniform 级别的抠取；
- 想要"先 baseline 复现、再独立工程化"的完整带闸流程。

产物（最小可复现的 baseline + 证据包）落在 `当前项目目录/` 内，
按本 skill 的产物规范补 NOTES/TEARDOWN，并继续走 Step 5 验证 + Step 6 替换。

若用户环境里已经有专门的 shader/帧捕获工具，可以把这份证据包交给它处理；没有也完全不影响——
上面四节的纪律本身就是可执行的做法。
