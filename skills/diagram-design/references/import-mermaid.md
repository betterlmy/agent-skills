...
Load this file for `.mmd`, `.mermaid`, or Markdown containing fenced `mermaid` blocks when the user asks to convert, redraw, simplify, or present the diagram. Some plugin hosts may provide a slash command that routes here, but it is optional.
