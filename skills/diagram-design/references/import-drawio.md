...
Load this file when the user points at a `.drawio`, `.drawio.xml`, `.drawio.png`, or `.drawio.svg` file and wants a diagram out of it — "convert this drawio", "redraw this diagram", "make this presentable", or "この drawio をきれいにして". Some plugin hosts may provide a slash command that routes here, but it is optional.
