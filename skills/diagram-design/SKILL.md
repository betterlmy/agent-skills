---
name: diagram-design
description: Create branded editorial diagrams as standalone HTML/SVG/PNG, including architecture, flowchart, sequence, state, data, timeline, quadrant, loop, process, and chart layouts. Use when a user wants a polished visual deliverable, asks to redraw a Mermaid or Draw.io source into a branded layout, needs accessible SVG or optional motion, or wants output sized for documents, slides, print, or social cards. Do not use when the primary deliverable is editable Draw.io XML or maintainable Mermaid source.
license: MIT
external-cli: true
...
- Quick text-only diagrams → use a compact text representation instead.
...
Before invoking Playwright or a browser CLI, read [`references/cli-compatibility.md`](references/cli-compatibility.md). The standalone Skill does not assume that plugin slash-command files are installed.
