...
description: 创建、审查和改进用户级与项目级 AGENTS.md，使编码 Agent 获得真实、明确、可执行且可验证的仓库协作规则。Use when 用户要求生成 AGENTS.md、检查现有 Agent 指令、整理项目开发约束、处理多级指令作用域，或验证 AGENTS.md 是否与代码仓库现状一致。
---
...
- 先区分仓库入口、用户全局规则和参考资料；全局规则不用填项目技术栈，参考资料不套仓库章节与行数模板。
- 当前任务已授权改进时完成必要修改与验证，不在每个步骤重复确认；范围外副作用仍须授权。
- 脚本默认 `--scope repository`；用户全局或 Pi 全局用 `--scope global`，技能内参考手册用 `--scope reference`。作用域按用途选择，不为了通过检查改变类型。
- 对生效指令同时检查字节体积；超过 16 KiB 会告警，不能靠压缩为长行规避上下文预算。
...
按对象类型检查章节、行数和字节预算；不要给用户全局文件补空的仓库章节。
...
仓库模式将缺少最低章节或超过 240 行报告为失败；global 保留安全和长度检查，reference 保留安全检查。
