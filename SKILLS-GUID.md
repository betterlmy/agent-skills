...
| 获取实时网页信息、查新闻/版本/事实、读文章全文、收集文档站章节、出带引用研究报告 | `tavily-web` | 先配置 `TAVILY_API_KEY` 环境变量并跑 `tv check`；离线任务或不依赖外部信息的任务不使用；无网络时降级到本地信息并声明新鲜度边界 |
| 复刻/克隆网站，或还原某个网页交互效果（WebGL/Canvas/Three.js） | `web-clone` | 先拿真源码，绝不信 AI 推测的代码；脚本需 Node.js 与系统浏览器，`gh` 缺失时降级为 GitHub 网页搜索；不用于完整业务系统开发 |
